...
# Make sure this points to your new model file
...
# Define local upsample to avoid importing from old modules
...
def parse_args():
    parser = ArgumentParser(description='Training PICA_ResNet_FPN')
    parser.add_argument('--base-size', type=int, default=256)  # Changed to 512 as requested
    parser.add_argument('--crop-size', type=int, default=256)
    parser.add_argument('--dataset', type=str, default='sirstaug')
    parser.add_argument('--batch-size', type=int, default=4)    # Changed to 2 as requested
    ...
    # Changed default name to match the new architecture
    parser.add_argument('--net-name', type=str, default='pica_resnet_fpn')
    
    parser.add_argument('--save-iter-step', type=int, default=1)
    parser.add_argument('--log-per-iter', type=int, default=40)
    ...
    args.logger = setup_logger("Prior_Net", args.save_folder, 0, filename='log.txt')
    return args
...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.dataset == 'nudt':
            ...
            self.base_data_dir = './datasets/sirst_aug'
        else:
        ...
        # CHANGED: FPN P5 has 256 channels, we project it to 8 channels to supervise CP2
        self.channel_proj = nn.Conv2d(in_channels=256, out_channels=8, kernel_size=1, stride=1, padding=0).to(self.device)
    ...
    # CHANGED: Adapted to FPN multi-scale outputs (d0, d2, d3, d4, d5)
    def muti_bce_loss_fusion(self, d0, d2, d3, d4, d5, labels):
        ...
        total_loss_bce = loss0 + loss2 + loss3 + loss4 + loss5
        ...
        for epoch in range(self.args.epochs):
            ...
            for i, (data, labels) in enumerate(self.train_data_loader):
                ...
                with amp.autocast('cuda'):
                    # Forward (Unpacking FPN outputs + priors + p5 deep feature)
                    d0, d2, d3, d4, d5, cp1, cp2, p5 = self.net(data) 
                    pred = torch.sigmoid(d0)

                    # Main segmentation losses
                    loss0, loss_bce = self.muti_bce_loss_fusion(d0, d2, d3, d4, d5, labels)
                    loss_iou = self.softiou(pred, labels)
                    
                    # CP1 Loss (Global structural prior constraint)
                    cp1_soft_gt = generate_soft_prior(labels)
                    cp1_loss = self.cp1_loss_fn(cp1, cp1_soft_gt) * self.args.cp1_weight
                    
                    # CP2 Loss (Deep semantic prior constraint, using p5 FPN feature)
                    cp2_ref = _upsample_like(p5, cp2)
                    ...
                    # FIXED: Added cp1_loss into the total cp loss calculation
                    loss_cp = cp2_loss
                    
                    # Total loss
                    total_loss = 10.0 * loss_iou + 10.0 * loss_bce + 10.0 * loss_cp
                
                # AMP Scaled Backward Pass and Step
                ...
                if self.iter_num % self.args.log_per_iter == 0:
                    ...
                    self.logger.info(base_log.format(
                        ...
                        float(loss_bce.detach().item()), float(loss_iou.detach().item()), 
                        float(cp2_loss.detach().item()), float(total_loss.detach().item()), 
