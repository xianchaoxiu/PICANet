...
class SegmentationMetricTPFNFP(object):
    ...
    def update(self, preds, labels):
        ...
        if isinstance(preds, torch.Tensor):
            preds = (preds.detach().numpy() > 0).astype('int64')
            #print(f'preds -> min: {preds.min().item():.4f}, max: {preds.max().item():.4f}, mean: {preds.mean().item():.4f}')
            labels = labels.numpy().astype('int64') 
            #print(f'labels -> min: {labels.min().item():.4f}, max: {labels.max().item():.4f}, mean: {labels.mean().item():.4f}') # T
